app.controller("PlatformNewRegionController", function($scope, $rootScope,
		Region, IngestRegion, usSpinnerService) {
	$scope.map = {
		center : {
			latitude : 45.65288,
			longitude : 25.61184
	...
	$scope.markerEvents = {
		drag : function(map, eventName, originalEventArgs) {
			console.info("Marker moved...", originalEventArgs.getGMarker().key,
					originalEventArgs.getGMarker().position.lat(),
					originalEventArgs.getGMarker().position.lng())
	...
	$scope.saveRegion = function() {
		...
		if ($scope.region.coords.points.length < 3) {
			var notEnoughpointsAlert = {
				type : 'danger',
				msg : "You must have at least 3 points selected.",
				id : "NotEnoughPoints"
			};
		...
		IngestRegion.post({}, params).then(function (result) {
			$scope.stopSpin();
			var successAlert = {
				 type: 'info', msg: "Data about this new region was ingested withsuccess.", id: "Success"
			};
			$scope.alerts.push(successAlert);
		 }, function (response) {
			 var msg = "Failed to ingest data. "+response.status+" "+response.statusText;
			 var failedAlert = {
	...
	var nmeIsEmptyAlert = {
		type : 'danger',
		msg : "You must give a name to the region.",
		id : "NameIsEmpty"
	};
